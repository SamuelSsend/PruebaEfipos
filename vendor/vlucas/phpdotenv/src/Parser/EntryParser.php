...
final class EntryParser
{
    ...
    {
        return self::splitStringIntoParts($entry)->flatMap(
            static function (array $parts) {
                [$name, $value] = $parts;

                return self::parseName($name)->flatMap(
                    static function (string $name) use ($value) {
                        /**
                    * @var Result<Value|null,string> 
                        */
                        ...
                        return $parsedValue->map(
                            static function (?Value $value) use ($name) {
...
*/
        $result = Str::pos($line, '=')->map(
            static function () use ($line) {
...
*/
        ...
        return \array_reduce(
            \iterator_to_array(Lexer::lex($value)), static function (Result $data, string $token) {
                return $data->flatMap(
                    static function (array $data) use ($token) {
                        return self::processToken($data[1], $token)->map(
                            static function (array $val) use ($data) {
                                return [$data[0]->append($val[0], $val[1]), $val[2]];
                            }
                        );
                    }
                );
            }, Success::create([Value::blank(), self::INITIAL_STATE])
        )->flatMap(
            ...
            * @var \GrahamCampbell\ResultType\Result<\Dotenv\Parser\Value,string> 
            ...
            static function (string $err) use ($value) {
