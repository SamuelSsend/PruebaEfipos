...
class BusFake implements QueueingDispatcher
{
    ...
    {
        $chain = collect($expectedChain)->map(
            function ($job) {
    ...
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(
            function ($chain) {
            ...
            function ($chain) use ($expectedChain) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return collect($this->commandsSync[$command])->filter(
            function ($command) use ($callback) {
                return $callback($command);
            }
        );
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return collect($this->commandsAfterResponse[$command])->filter(
            function ($command) use ($callback) {
                return $callback($command);
            }
        );
    ...
    public function batched(callable $callback)
    {
        ...
        return collect($this->batches)->filter(
            function ($batch) use ($callback) {
