...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    public static function render($string, $data = [], $deleteCachedView = false)
    {
        ...
        return tap(
            $view->render(), function () use ($view, $deleteCachedView) {
                if ($deleteCachedView) {
                    unlink($view->getPath());
                }
            }
    ...
    protected function restoreRawContent($result)
    {
        $result = preg_replace_callback(
            '/'.$this->getRawPlaceholder('(\d+)').'/', function ($matches) {
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive(
            $name, function ($expression) use ($name) {
                return $expression !== ''
        ...
        $this->directive(
            'unless'.$name, function ($expression) use ($name) {
                return $expression !== ''
                ? "<?php if (! \Illuminate\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php if (! \Illuminate\Support\Facades\Blade::check('{$name}')): ?>";
            }
        ...
        $this->directive(
            'else'.$name, function ($expression) use ($name) {
                return $expression !== ''
    ...
    public function component($class, $alias = null, $prefix = '')
    {
        ...
        if (is_null($alias)) {
            $alias = Str::contains($class, '\\View\\Components\\')
                            ? collect(explode('\\', Str::after($class, '\\View\\Components\\')))->map(
                                function ($segment) {
    ...
    public function aliasComponent($path, $alias = null)
    {
        ...
        $this->directive(
            $alias, function ($expression) use ($path) {
                return $expression
