...
abstract class Factory
{
    ...
    public function raw($attributes = [], ?Model $parent = null)
    {
        ...
        return array_map(
            function () use ($attributes, $parent) {
                return $this->state($attributes)->getExpandedAttributes($parent);
            }, range(1, $this->count)
        );
    }
    ...
    public function make($attributes = [], ?Model $parent = null)
    {
        ...
        if ($this->count === null) {
            return tap(
                $this->makeInstance($parent), function ($instance) {
    ...
    protected function makeInstance(?Model $parent)
    {
        return Model::unguarded(
            function () use ($parent) {
                return tap(
                    $this->newModel($this->getExpandedAttributes($parent)), function ($instance) {
                        if (isset($this->connection)) {
    ...
    protected function parentResolvers()
    {
        ...
        return $this->for->map(
            function (BelongsToRelationship $for) use ($model) {
    ...
    {
        $instances->each(
            function ($model) use ($parent) {
                $this->afterCreating->each(
                    function ($callback) use ($model, $parent) {
