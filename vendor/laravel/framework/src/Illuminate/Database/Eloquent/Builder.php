...
class Builder
{
    ...
    public function hydrate(array $items)
    {
        ...
        return $instance->newCollection(
            array_map(
                function ($item) use ($items, $instance) {
    ...
    public function firstOrCreate(array $attributes = [], array $values = [])
    {
        ...
        return tap(
            $this->newModelInstance(array_merge($attributes, $values)), function ($instance) {
                $instance->save();
            }
        );
    }
    ...
    {
        return tap(
            $this->firstOrNew($attributes), function ($instance) use ($values) {
    ...
    {
        return $this->applyScopes()->query->cursor()->map(
            function ($record) {
    ...
    protected function ensureOrderForCursorPagination($shouldReverse = false)
    {
        ...
        if ($shouldReverse) {
            $this->query->orders = collect($this->query->orders)->map(
                function ($order) {
    ...
    {
        return tap(
            $this->newModelInstance($attributes), function ($instance) {
                $instance->save();
            }
        );
    }
    ...
    protected function createSelectWithConstraint($name)
    {
        return [explode(':', $name)[0], static function ($query) use ($name) {
            $query->select(
                array_map(
                    static function ($column) use ($query) {
