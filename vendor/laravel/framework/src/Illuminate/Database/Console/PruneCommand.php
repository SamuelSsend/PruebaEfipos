...
class PruneCommand extends Command
{
    ...
    public function handle(Dispatcher $events)
    {
        ...
        $events->listen(
            ModelsPruned::class, function ($event) {
    ...
    protected function models()
    {
        ...
        return collect((new Finder)->in($this->getDefaultPath())->files()->name('*.php'))
            ->map(
                ...
                ! empty($except), function ($models) use ($except) {
                        return $models->reject(
                            function ($model) use ($except) {
    ...
    protected function pretendToPrune($model)
    {
        ...
        $count = $instance->prunable()
            ->when(
                in_array(SoftDeletes::class, class_uses_recursive(get_class($instance))), function ($query) {
                    $query->withTrashed();
                }
            )->count();
