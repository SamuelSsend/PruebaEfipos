...
class DynamoDbStore implements LockProvider, Store
{
    ...
    public function many(array $keys)
    {
        ...
        return array_merge(
            collect(array_flip($keys))->map(
                function () {
                    //
                }
            )->all(), collect($response['Responses'][$this->table])->mapWithKeys(
                function ($response) use ($now) {
    ...
    public function putMany(array $values, $seconds)
    {
        ...
        $this->dynamo->batchWriteItem(
            [
            'RequestItems' => [
                $this->table => collect($values)->map(
                    function ($value, $key) use ($expiration) {
                        return [
                        'PutRequest' => [
                            'Item' => [
                                $this->keyAttribute => [
