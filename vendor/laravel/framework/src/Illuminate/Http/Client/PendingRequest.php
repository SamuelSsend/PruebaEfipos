...
class PendingRequest
{
    ...
    {
        return tap(
            $this, function ($request) use ($format) {
    ...
    {
        return tap(
            $this, function ($request) use ($headers) {
    ...
    {
        return tap(
            $this, function ($request) use ($username, $password) {
    ...
    {
        return tap(
            $this, function ($request) use ($token, $type) {
    ...
    {
        return tap(
            $this, function ($request) use ($userAgent) {
    ...
    {
        return tap(
            $this, function ($request) use ($cookies, $domain) {
                return $this->options = array_merge_recursive(
                    $this->options, [
    ...
    {
        return tap(
            $this, function ($request) use ($options) {
    ...
    public function dump()
    {
        ...
        return $this->beforeSending(
            function (Request $request, array $options) use ($values) {
    ...
    public function send(string $method, string $url, array $options = [])
    {
        ...
        return retry(
            $this->tries ?? 1, function () use ($method, $url, $options) {
                try {
                    return tap(
                        new Response($this->sendRequest($method, $url, $options)), function ($response) {
        ...
        return $this->promise = $this->sendRequest($method, $url, $options)
            ->then(
                function (MessageInterface $message) {
                    return tap(
                        new Response($message), function ($response) {
                            $this->populateResponse($response);
                            $this->dispatchResponseReceivedEvent($response);
                        }
                    );
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        return $this->buildClient()->$clientMethod(
            $method, $url, $this->mergeOptions(
                [
    ...
    {
        return tap(
            $handlerStack, function ($stack) {
                $stack->push($this->buildBeforeSendingHandler());
                $stack->push($this->buildRecorderHandler());
                $stack->push($this->buildStubHandler());

                $this->middleware->each(
                    function ($middleware) use ($stack) {
    ...
    {
        return tap(
            $request, function (&$request) use ($options) {
                $this->beforeSendingCallbacks->each(
                    function ($callback) use (&$request, $options) {
