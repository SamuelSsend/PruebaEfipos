...
class IgnitionServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(
            SolutionProviderRepositoryContract::class, function () {
    ...
    protected function registerExceptionRenderer()
    {
        if (interface_exists(\Whoops\Handler\HandlerInterface::class)) {
            $this->app->bind(
                \Whoops\Handler\HandlerInterface::class, function (Application $app) {
        ...
        if (interface_exists(\Illuminate\Contracts\Foundation\ExceptionRenderer::class)) {
            $this->app->bind(
                \Illuminate\Contracts\Foundation\ExceptionRenderer::class, function (Application $app) {
    ...
    {
        $this->app->singleton(
            LogRecorder::class, function (Application $app): LogRecorder {
    ...
    {
        $this->app->singleton(
            QueryRecorder::class, function (Application $app): QueryRecorder {
    ...
    protected function registerBuiltInMiddleware()
    {
        ...
        $middleware = collect($middlewares)
            ->map(
                function (string $middlewareClass) {
    ...
    {
        $this->app['log']->listen(
            function (MessageLogged $messageLogged) {
                if (config('flare.key')) {
                    try {
    ...
    protected function setupOctane()
    {
        $this->app['events']->listen(
            RequestReceived::class, function () {
                $this->resetFlare();
            }
        );

        $this->app['events']->listen(
            TaskReceived::class, function () {
                $this->resetFlare();
            }
        );

        $this->app['events']->listen(
            TickReceived::class, function () {
                $this->resetFlare();
            }
        );
    }
