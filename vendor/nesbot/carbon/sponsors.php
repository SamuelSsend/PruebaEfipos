...
function getOpenCollectiveSponsors(): string
{
    ...
    $list = array_filter(
        $members, static function ($member): bool {
    ...
    $list = array_map(
        static function (array $member): array {
            ...
            if ($lastTransactionAt->isAfter('last month') 
    ...
    usort(
        $list, static function (array $a, array $b): int {
    ...
    return implode(
        '', array_map(
            static function (array $member) use ($customSponsorImages): string {
...
file_put_contents(
    'readme.md', preg_replace_callback(
