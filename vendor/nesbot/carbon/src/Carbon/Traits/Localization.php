...
trait Localization
{
    ...
    public static function translateTimeString($timeString, $from = null, $to = null, $mode = CarbonInterface::TRANSLATE_ALL)
    {
        ...
        foreach (['from', 'to'] as $key) {
            ...
            $$translationKey = array_merge(
                ...
                $mode & CarbonInterface::TRANSLATE_MERIDIEM ? array_map(
                    function ($hour) use ($meridiem) {
        ...
        return substr(
            preg_replace_callback(
                '/(?<=[\d\s+.\/,_-])('.implode('|', $fromTranslations).')(?=[\d\s+.\/,_-])/iu', function ($match) use ($fromTranslations, $toTranslations) {
                    [$chunk] = $match;
        ...
        if (method_exists($translator, 'setFallbackLocales')) {
            ...
            if ($translator instanceof Translator) {
                ...
                $translator->setMessages(
                    $preferredLocale, array_replace_recursive(
    ...
    {
        return static::executeWithLocale(
            $locale, function ($newLocale, TranslatorInterface $translator) {
    ...
    {
        return array_map(
            function ($wordKey) use ($messages, $key) {
    ...
    private static function replaceOrdinalWords(string $timeString, array $ordinalWords): string
    {
        return preg_replace_callback(
            '/(?<![a-z])[a-z]+(?![a-z])/i', function (array $match) use ($ordinalWords) {
