    ...
    public static function __callStatic($method, $parameters)
    {
        try {
            ...
            $result = static::hasMacro($method)
                ? static::bindMacroContext(
                    null, function () use (&$method, &$parameters, &$interval) {
...
*/
    ...
    {
        $interval = @parent::createFromDateString(
            strtr(
                $time, [
                ',' => ' ',
                ' and ' => ' ',
    ...
    public function __call($method, $parameters)
    {
        if (static::hasMacro($method)) {
            return static::bindMacroContext(
                $this, function () use (&$method, &$parameters) {
    ...
    protected function getForHumansParameters($syntax = null, $short = false, $parts = -1, $options = null)
    {
        ...
        $skip = array_map(
            'strtolower', array_filter(
                (array) $skip, static function ($value) {
...
*/
    ...
    public static function getDateIntervalSpec(DateInterval $interval, bool $microseconds = false, array $skip = [])
    {
        ...
        if ($interval->days >= CarbonInterface::DAYS_PER_WEEK * CarbonInterface::WEEKS_PER_MONTH 
