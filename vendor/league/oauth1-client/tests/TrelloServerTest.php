...
class TrelloTest extends TestCase
{
    ...
    public function testGettingTemporaryCredentials()
    {
        ...
        $client->shouldReceive('post')->with(
            'https://trello.com/1/OAuthGetRequestToken', m::on(
                function ($options) use ($me) {
    ...
    public function testGettingTokenCredentials()
    {
        ...
        $me = $this;
        $client->shouldReceive('post')->with(
            'https://trello.com/1/OAuthGetAccessToken', m::on(
                function ($options) use ($me) {
                    $headers = $options['headers'];
    ...
    public function testGettingUserDetails()
    {
        ...
        $client->shouldReceive('get')->with(
            'https://trello.com/1/members/me?key=' . $this->getApplicationKey() . '&token=' . $this->getAccessToken(), m::on(
                function ($options) use ($me) {
                    $headers = $options['headers'];
