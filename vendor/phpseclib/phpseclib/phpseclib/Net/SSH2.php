...
*/
    ...
    private function key_exchange($kexinit_payload_server = false)
    {
        ...
        case substr($this->server_identifier, 0, 13) == 'SSH-2.0-DLINK':
            if (!isset($preferred['server_to_client']['mac'])) {
                $s2c_mac_algorithms = array_values(
                    array_diff(
                        $s2c_mac_algorithms,
            ...
            if (!isset($preferred['client_to_server']['mac'])) {
                $c2s_mac_algorithms = array_values(
                    array_diff(
                        $c2s_mac_algorithms,
                        ['hmac-sha1-96', 'hmac-md5-96']
                    )
        ...
        case substr($this->server_identifier, 0, 24) == 'SSH-2.0-TurboFTP_SERVER_':
            if (!isset($preferred['server_to_client']['crypt'])) {
                $s2c_encryption_algorithms = array_values(
                    array_diff(
            ...
            if (!isset($preferred['client_to_server']['crypt'])) {
                $c2s_encryption_algorithms = array_values(
                    array_diff(
    ...
    private function get_binary_packet_size(&$packet)
    {
        ...
        if ($packet_length <= 0 || $packet_length > 0x9000
     ...
     *                              on other channels is buffered. The respective negative
     *                              value of a channel is also supported for the case that the
     *                              caller is awaiting adjustment of the data window, and where
     *                              data received on that respective channel is also buffered.
     * @param  bool $skip_extended
     * @return mixed
     * @throws \RuntimeException on connection error
     */
